"""setup1_v2b_4h.py — v2.b trigger test: 2019-01-01 sampai sekarang, 4H TF.
SL: ATR%, Liq10x. TP: 3x. RAWBRK tracked.
"""
import sys, os, time
sys.path.insert(0, "/home/ubuntu/trading-research")
sys.path.insert(0, "/home/ubuntu/trading-research/indicators")
sys.path.insert(0, "/home/ubuntu/trading-research/backtests/BTCUSDT/SETUP1/4H")
...
# fetch
CACHE=os.path.join(HERE,"data_BTCUSDT_4h_2019now.csv")
...
print(f"[*] loading BTCUSDT 4h 2019..now")
...
# ── RUN BACKTEST ──
print(f"\n{'='*80}")
print(f"SETUP1 v2.b BACKTEST 4H — 2019-01-01 to {END.strftime('%Y-%m-%d')}")
...
print(f"\nSELESAI — {len(samples)} signals, {df.index[0]} to {df.index[-1]}")
