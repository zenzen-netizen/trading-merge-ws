"""setup1_v2b_2023.py — v2.b trigger test: 2023-01-01 sampai sekarang.
...
# fetch
CACHE2023=os.path.join(HERE,"data_BTCUSDT_1d_2023now.csv")
...
print(f"[*] loading BTCUSDT 1d 2023..now")
if os.path.exists(CACHE2023) and (time.time()-os.path.getmtime(CACHE2023))<86400:
    df=pd.read_csv(CACHE2023,index_col=0,parse_dates=True)
...
# ── RUN BACKTEST ──
print(f"\n{'='*80}")
print(f"SETUP1 v2.b BACKTEST — 2023-01-01 to {END.strftime('%Y-%m-%d')}")
print(f"SL: ATR%% / Liq10x   |   TP: 3x   |   EXIT: RAWBRK only (close<PL→SL=HIGH)")
print(f"{'='*80}")
...
for sl_name, sl_fn in [
    ...
    print(f"  TP(3x): {tp_count:>3}  |  SL: {sl_count:>3}  |  RAWBRK_HIT: {rb_count:>3}  |  EXP: {exp_count:>3}")
    print(f"  Sum R: {totR:+.2f}  |  WR(TP/all): {wr:.1f}%  |  RAWBRK_HIT R: {rb_hitR:+.2f}")
