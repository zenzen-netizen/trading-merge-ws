"""
_whatif_reverse_long.py — v2.b trigger 1H, posisi LONG (REVERSE).
Trigger IDENTIK v2.b SHORT (FBF bear + SMI FMB→PD→XDN + ST DOWN).
Yang berubah: ARAH posisi jadi LONG (counter-trend play).
SL di BAWAH entry, TP di ATAS entry, RAWBRK pake pivot HIGH.
"""
import sys, os, time
...
# ── PHASE 3: v2.b trigger (IDENTIK dgn SHORT) ──
samples=[]
...
# ── PHASE 4: BACKTEST (LONG) ──

# Pivot HIGHS for LONG rawbreak
def find_pivot_highs(arr,left=3,right=3):
    out=[]
    ...
    return out
ph_arr=find_pivot_highs(high,3,3)
ph_dict={bar:val for val,bar in ph_arr}

# ── LONG SIMULATORS ──

def sim_rawbrk_long(entry, eb, sl, tp_mult):
    """LONG: sl < entry, tp = entry + n*risk, rawbreak = close > pivotHigh.
    RAWBRK trigger: close > lastPivotHigh → rb_sl = LOW candle.
    RAWBRK HIT: low <= rb_sl."""
    risk=entry-sl
    ...
    for k in range(len(fwd_h)):
        ...
        if rb_sl is not None and fwd_l[k]<=rb_sl:
            return dict(out="RAWBRK_HIT",ex=rb_sl,bars=k+1,R=(rb_sl-entry)/risk,
        ...
        if fwd_l[k]<=sl:
            return dict(out="SL",ex=sl,bars=k+1,R=-1.0,
                       rawbreak_bars=rb_bars,rawbreak_sl=rb_sl,rawbreak_price=None)
        # RAWBRK trigger: close > lastPivotHigh
        cur_ph=None
        for bi2 in sorted(ph_dict.keys(),reverse=True):
            if bi2<=bi: cur_ph=ph_dict[bi2]; break
        if cur_ph is not None and close[bi]>cur_ph:
            if rb_sl is None: rb_sl=low[bi]
            else: rb_sl=min(rb_sl, low[bi])  # tighter
            rb_bars.append((bi,low[bi]))
    
    lastc=fwd_h[-1] if len(fwd_h)>0 else entry
    return dict(out="EXP",ex=lastc,bars=len(fwd_h),R=(lastc-entry)/risk,
               rawbreak_bars=rb_bars,rawbreak_sl=rb_sl,rawbreak_price=None)

def sim_st_rev_long(entry, eb, sl):
    """LONG: ST_REV = exit when ST flips DOWN (+1→-1)."""
    risk=entry-sl
    tp3=entry+3*risk
    fwd_h=high[eb+1:]; fwd_l=low[eb+1:]
    ...
    for k in range(len(fwd_h)):
        ...
        if fwd_l[k]<=sl:
            return dict(out="SL",ex=sl,bars=k+1,R=-1.0)
        if st_trend[bi]==-1:  # ST flipped DOWN
            return dict(out="ST_REV",ex=close[bi],bars=k+1,R=(close[bi]-entry)/risk)
    
    lastc=fwd_h[-1] if len(fwd_h)>0 else entry
    return dict(out="EXP",ex=lastc,bars=len(fwd_h),R=(lastc-entry)/risk)

# ── SL options (LONG: sl BELOW entry) ──
def sl_o1(s): return s["entry"]*0.90
...
print(f"\n{'='*80}")
print(f"SETUP1 v2.b REVERSE LONG — 1H — 2019-01-01 to {END.strftime('%Y-%m-%d')}")
print(f"(Trigger v2.b SAMA, arah LONG counter-trend)")
print(f"{'='*80}")
...
for sl_name, sl_fn in SL_OPTIONS:
    ...
    # TP1x + RAWBRK
    tp1x_res=[sim_rawbrk_long(s["entry"],s["bar"],sl,1.0) for s,sl in valid]
    ...
    for s,sl in valid:
        ...
        for k in range(len(fwd_h)):
            ...
            if fwd_l[k]<=sl:
                ...
                found=True; break
        if not found:
            lastc=fwd_h[-1] if len(fwd_h)>0 else s["entry"]
            tp3x_res.append(dict(out="EXP",R=(lastc-s["entry"])/risk,bars=len(fwd_h)))
    ...
    # RAWBRK (TP3x + RAWBRK)
    rb_res=[sim_rawbrk_long(s["entry"],s["bar"],sl,3.0) for s,sl in valid]
    ...
    # ST_REV (TP3x + ST flip)
    sr_res=[sim_st_rev_long(s["entry"],s["bar"],sl) for s,sl in valid]
...
# ── CROSS-TF COMPARISON ──
print(f"\n{'='*80}")
print(f"CROSS-TF: SHORT vs LONG (RAWBRK + ST_REV)")
print(f"{'='*80}")

# SHORT known results (1H from earlier session)
SHORT_RAWBRK={"O1":-52.88,"O3":-88.08,"O6":-118.39}
SHORT_STREV={"O1":-23.28,"O3":-65.50,"O6":-69.83}

print(f"\n--- TP3x+RAWBRK ---")
print(f"{'Dir':>6} {'O1 Liq10x':>12} {'O3 ATR%c':>12} {'O6 ATR%1D':>12}")
print(f"{'':->6} {'':->12} {'':->12} {'':->12}")
print(f"{'SHORT':>6} {SHORT_RAWBRK['O1']:>+12.2f} {SHORT_RAWBRK['O3']:>+12.2f} {SHORT_RAWBRK['O6']:>+12.2f}")
l1={r["sl"].split()[0]:r["rawbrk"]["tot"] for r in ALL_RESULTS}
print(f"{'LONG':>6} {l1.get('O1',0):>+12.2f} {l1.get('O3',0):>+12.2f} {l1.get('O6',0):>+12.2f}")

print(f"\n--- ST_REV ---")
print(f"{'Dir':>6} {'O1 Liq10x':>12} {'O3 ATR%c':>12} {'O6 ATR%1D':>12}")
print(f"{'':->6} {'':->12} {'':->12} {'':->12}")
print(f"{'SHORT':>6} {SHORT_STREV['O1']:>+12.2f} {SHORT_STREV['O3']:>+12.2f} {SHORT_STREV['O6']:>+12.2f}")
l2={r["sl"].split()[0]:r["strev"]["tot"] for r in ALL_RESULTS}
print(f"{'LONG':>6} {l2.get('O1',0):>+12.2f} {l2.get('O3',0):>+12.2f} {l2.get('O6',0):>+12.2f}")
...
for s in samples[-5:]:
    wib=fmt_wib(s["date"])
    das=s.get("daily_apct")
    if das is not None and not np.isnan(das):
        print(f"  {wib} | entry={s['entry']:.2f} | apct={s['apct']:.2f}% | daily_apct={das:.2f}%")
    else:
        print(f"  {wib} | entry={s['entry']:.2f} | apct={s['apct']:.2f}% | daily_apct=None")

print(f"\nSELESAI — {len(samples)} signals, {df.index[0]} to {df.index[-1]} UTC")
print(f"WHAT-IF: v2.b bearish signal → LONG counter-trend.")
print(f"Spec: TRIGGER_SPEC_v2b_REVERSE.md + SETUP_SPEC_v2b_REVERSE_LONG.md")
