...
DISCOVERY_JSON = ROOT / "filtered-top20-sol-30m-fee-tvl-age15h-mcap250k-vol10pct.json"
OUTDIR = ROOT
OUTDIR.mkdir(parents=True, exist_ok=True)

NETWORK = "solana"
METEORA_OHLCV_BASE = "https://dlmm.datapi.meteora.ag/pools"
MERIDIAN_CHART_API = os.getenv("MERIDIAN_CHART_API_URL", "https://api.agentmeridian.xyz/api").rstrip("/")
MERIDIAN_PUBLIC_API_KEY = os.getenv("MERIDIAN_PUBLIC_API_KEY", "")  # redacted for public mirror
TFS = {
    ...
    "30m": {"timeframe": "minute", "aggregate": 30, "minutes": 30, "max_pages": 3},
}
LIMIT = 1000
RSI_LEN = 2
FRACTAL_N = 3
...
MIN_REQUIRED_CANDLES = max(EMA_FAST + FRACTAL_N + 5, 70)
BE_MODE_SPECS = [
...
def chart_headers() -> dict[str, str]:
    headers = {"accept": "application/json"}
    if MERIDIAN_PUBLIC_API_KEY:
        headers["x-api-key"] = MERIDIAN_PUBLIC_API_KEY
    return headers


def fetch_ohlcv(pool: str, tf: str) -> list[dict[str, float]]:
    if tf == "30m":
        url = f"{METEORA_OHLCV_BASE}/{pool}/ohlcv"
        r = requests.get(url, params={"timeframe": "30m"}, timeout=30, headers={"accept": "application/json"})
        r.raise_for_status()
        payload = r.json()
...
def fetch_jupiter_15m_ohlcv(mint: str, candles: int = 298) -> list[dict[str, float]]:
    url = f"{MERIDIAN_CHART_API}/chart-indicators/{mint}"
    params = {"interval": "15_MINUTE", "candles": candles, "rsiLength": RSI_LEN}
    last_err = None
    for attempt in range(3):
        r = requests.get(url, params=params, timeout=30, headers=chart_headers())
        if r.ok:
            payload = r.json()
    ...
    raise RuntimeError(f"Jupiter proxy fetch failed for {mint}: {last_err}")


def aggregate_15m_to_30m(rows: list[dict[str, float]]) -> list[dict[str, float]]:
    by_ts = {int(r["ts"]): r for r in sorted(rows, key=lambda r: int(r["ts"]))}
    ...
    for ts in sorted(by_ts):
        ...
        out.append({
            ...
            "close": float(b["close"]),
            "volume": float(a.get("volume") or 0.0) + float(b.get("volume") or 0.0),
        })
    return out
...
def simulate(rows: list[dict[str, float]], pool_name: str, pool_addr: str, tf: str, candle_source: str) -> list[dict[str, Any]]:
    n = len(rows)
...
def summarize(trades: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (pool_addr, tf, opt, exit_mode), arr in sorted(groups.items()):
        ...
        candle_source = arr[0].get("candle_source", "")
...
def cache_path_for(pool_name: str, pool_addr: str, tf: str, suffix: str = "") -> Path:
    return OUTDIR / f"ohlcv_{safe_name(pool_name)}_{pool_addr[:8]}_{tf}{suffix}.csv"


def get_rows_with_fallback(pool_addr: str, mint: str | None, pool_name: str, tf: str) -> tuple[list[dict[str, float]], str]:
    canonical_path = cache_path_for(pool_name, pool_addr, tf)
    if canonical_path.exists() and canonical_path.stat().st_size > 0:
        canonical_rows = read_ohlcv_csv(canonical_path)
    else:
        canonical_rows = fetch_ohlcv(pool_addr, tf)
        write_csv(canonical_path, canonical_rows)

    if tf != "30m" or len(canonical_rows) >= MIN_REQUIRED_CANDLES:
        return canonical_rows, "pool_canonical"

    if not mint:
        return canonical_rows, "pool_canonical_insufficient_no_mint"

    proxy_path = cache_path_for(pool_name, pool_addr, tf, "_jupiter_proxy")
    try:
        if proxy_path.exists() and proxy_path.stat().st_size > 0:
            proxy_rows = read_ohlcv_csv(proxy_path)
        else:
            proxy_rows = aggregate_15m_to_30m(fetch_jupiter_15m_ohlcv(mint))
            write_csv(proxy_path, proxy_rows)
    except Exception as e:
        print(f"[warn] {pool_label(pool_name, pool_addr)} {tf}: jupiter proxy fetch failed: {e}", flush=True)
        return canonical_rows, "pool_canonical_insufficient_proxy_error"

    if len(proxy_rows) >= MIN_REQUIRED_CANDLES:
        return proxy_rows, "jupiter_proxy_15m_agg30m"
    return canonical_rows, "pool_canonical_insufficient_proxy_insufficient"


def md_report(summary: list[dict[str, Any]], trades: list[dict[str, Any]], data_counts: list[dict[str, Any]]) -> str:
    lines = []
    lines.append("# RSI(2) + Fractal(3) Backtest — Meteora SOL Pools (TF30m + MCAP250k + Vol10pct)")
    lines.append("")
    ...
    lines.append("30m fallback: when canonical pool candles are shorter than the EMA/fractal warmup requirement, use Jupiter-backed mint candles aggregated from 15m to synthetic 30m and label the source explicitly.")
    lines.append("")
    lines.append("Discovery variant: min base token market cap 250k; min volume/tvl ratio 0.10; age >= 15h; SOL pairs only; exclude USDC; top 20 by fee_active_tvl_ratio.")
    lines.append("")
    lines.append("## Data coverage")
    lines.append("| Pool | TF | Source | Candles | Start WIB | End WIB |")
    lines.append("|---|---|---|---:|---|---|")
    for d in data_counts:
        lines.append(f"| {d['pool']} | {d['tf']} | {d['source']} | {d['candles']} | {d['start']} | {d['end']} |")
    lines.append("")
    lines.append("## Summary")
    lines.append("| Pool | TF | Source | SL | Exit | Tr | TP | BE | BE% | SL | EXP | WR% | BE Gate Hit | BE Gate Rec | BE Gate% | Wstreak | Lstreak | AvgHold | SumR |")
    lines.append("|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for s in summary:
        lines.append(
            f"| {s['pool']} | {s['tf']} | {s.get('candle_source', '')} | {s['sl_opt']} | {s['exit_mode']} | {s['trades']} | {s['TP']} | {s['BE']} | {s['be_rate_pct']:.1f} | {s['SL']} | {s['EXP']} | "
    ...
    lines.append("")
    lines.append("## Notes")
    lines.append("- `pool_canonical` = canonical pool candles (Meteora 30m endpoint for 30m, GeckoTerminal pool candles for 1m/5m/15m).")
    lines.append("- `jupiter_proxy_15m_agg30m` = Jupiter-backed mint candles from Meridian chart-indicator API, aggregated from 15m into synthetic 30m because canonical pool 30m history was too short.")
...
def main():
    ...
    for p in pools:
        ...
        mint = (p.get("token_x") or {}).get("address")
        label = pool_label(name, pool)
        for tf in TFS:
            print(f"[fetch] {label} {tf} {pool}", flush=True)
            rows, candle_source = get_rows_with_fallback(pool, mint, name, tf)
            print(f"[source] {label} {tf}: {candle_source} ({len(rows)} candles)", flush=True)
            if rows:
                data_counts.append({
                    "pool": label,
                    "tf": tf,
            ...
            if len(rows) < MIN_REQUIRED_CANDLES:
                print(f"[skip] {label} {tf}: not enough candles for EMA50/fractal warmup (source={candle_source})", flush=True)
                continue
            all_trades.extend(simulate(rows, name, pool, tf, candle_source))
            time.sleep(REQ_SLEEP)

    summary = summarize(all_trades)
    write_csv(OUTDIR / "trade-journal.csv", all_trades)
