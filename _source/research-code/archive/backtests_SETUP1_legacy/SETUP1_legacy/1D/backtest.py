...
# bear_trigger: bars where bear_active just became active (False→True)
# = this is the RAWBRK CONFIRM candle (trigger event for trailing)
bear_trigger = np.zeros(len(close), dtype=bool)
for i in range(1, len(close)):
    if bear_active[i] and not bear_active[i-1]:
        bear_trigger[i] = True

print(f"[RAWBRK] bear active at last bar: {bear_active[-1]}, total bars break-active: {int(bear_active.sum())}, trigger events: {int(bear_trigger.sum())}")
...
# ---------- run ----------
# EXIT GROUPING (Level 2c): 3 MODE combo
# MODE A: SL + TP (1x/2x/3x)
# MODE B: SL + TP (1x/2x/3x) + TRAILING (ATR/ST/RB) backup
# MODE C: SL + TRAILING (ATR/ST/RB) aja
OPT_KEYS = ["O1","O2","O3","O4","O5"]
OPT_NAMES = {"O1":"Liq10x","O2":"C+ATR14","O3":"ATR%30","O4":"Fix6%","O5":"ConfHi+ATR"}
TP_RATIOS = ["1x","2x","3x"]
TRAIL_VARIANTS = ["ATR","ST","RB"]  # TRAIL-ATR / TRAIL-ST / TRAIL-RB

# Combo definitions: (mode_label, opt, tp_ratio, trail_variant)
COMBOS = []
for ok in OPT_KEYS:
    for tr in TP_RATIOS:
        COMBOS.append(("A", ok, tr, None))          # MODE A: SL+TP
for ok in OPT_KEYS:
    for tr in TP_RATIOS:
        for tv in TRAIL_VARIANTS:
            COMBOS.append(("B", ok, tr, tv))        # MODE B: SL+TP+TRAIL
for ok in OPT_KEYS:
    for tv in TRAIL_VARIANTS:
        COMBOS.append(("C", ok, None, tv))          # MODE C: SL+TRAIL

results = {}  # results[(mode, opt, tp, trail)] = list of (outcome, bars, R)
for c in COMBOS:
    results[c] = []


# ---------- simulation per combo (mode x opt x tp x trail) ----------
def run_batch(setups, opt_key, mode, tp_ratio, trail_variant):
    """Run all setups for one combo. Returns list of (outcome, bars, R).
    MODE A: SL+TP (fixed target)
    MODE B: SL+TP+TRAIL (TP patok + trailing backup)
    MODE C: SL+TRAIL aja (no TP)
    trail_variant: 'ATR'=ATR%, 'ST'=Supertrend, 'RB'=RawBreak
    """
    out = []
    for s in setups:
        ...
        for k in range(len(fwd_h)):
            ...
            if mode in ("B", "C"):
                if trail_variant == "ATR":
                    ...
                    # Trigger: bear_active False→True (rawbreak confirm candle)
...
# ---------- Collect all results + write journal CSV ----------
TRADE_DETAIL = []
results = {}  # (set_name, mode, opt, tp, tv) -> list of (outcome, bars, R)

for mode_label, ok, tp_ratio, trail_variant in COMBOS:
    for setup_set, set_name in [(setups, "RAW"), (gated, "GATED")]:
        res = run_batch(setup_set, ok, mode_label, tp_ratio, trail_variant)
        key = (set_name, mode_label, ok, tp_ratio, trail_variant)
        results[key] = res
        # journal: write each trade
        for idx, s in enumerate(setup_set):
            ...
            rec = {
                ...
                "tp_ratio": tp_ratio or "",
                "trail_variant": trail_variant or "",
            ...
            TRADE_DETAIL.append(rec)

import csv
with open("/home/ubuntu/trading-research/setup1_v3_journal.csv","w",newline="") as f:
    w = csv.DictWriter(f, fieldnames=["mode","opt","combo_mode","tp_ratio","trail_variant",
                                       "entry_date","entry","sl","sl_pct","tp_pct",
                                       "apct","outcome","bars","R"])
    w.writeheader(); w.writerows(TRADE_DETAIL)
print(f"[JOURNAL] wrote {len(TRADE_DETAIL)} trades -> setup1_v3_journal.csv")

# ---------- summary ----------
def summarize_v3(label, setup_set, set_name):
    print(f"\n{'='*72}")
    print(f"SETUP1 SHORT v3 — MODE A/B/C  [{label}]")
    ...
    for mode in ["A","B","C"]:
        ...
        for combo in COMBOS:
            ...
            avgR = totR/n
            if mode == "A":
                name = f"{ok}-{OPT_NAMES[ok]} {tp}"
            elif mode == "B":
                name = f"{ok}-{OPT_NAMES[ok]} {tp}+{tv}"
            else:
                name = f"{ok}-{OPT_NAMES[ok]} t-{tv}"
            print(f"{name:<20}{n:>5}{tp_n:>5}{sl_n:>5}{atr_n:>5}{srv:>5}{rbk:>5}{rtl:>5}{exp:>5}{wr:>5.1f}%{totR:>+8.1f}{avgR:>+8.3f}")

print("\n"+"="*72)
summarize_v3("RAW (unfiltered)", setups, "RAW")
print("\n"+"="*72)
summarize_v3("GATED (ST downtrend @ C + entry)", gated, "GATED")
print("\n"+"="*72)
...
if canon:
    print(f"\n━━━ CANONICAL 2026-06 (RAW, all combos) ━━━")
    ...
    for mode in ["A","B","C"]:
        for combo in COMBOS:
            ...
            rs = results.get(key, [])
            if not rs: continue
            ri = [x for x in range(len(setups)) if setups[x] is s]
            if not ri: continue
            r = rs[ri[0]]
            if r[0]=="INV": continue
            name = f"{ok}-{OPT_NAMES[ok]}"
            if mode == "A": name += f" {tp}"
            elif mode == "B": name += f" {tp}+{tv}"
            else: name += f" t-{tv}"
            print(f"  {name:<20} → {r[0]:>5} in {r[1]:>3}d  R={r[2]:+.2f}")
print("\n"+"="*72)
